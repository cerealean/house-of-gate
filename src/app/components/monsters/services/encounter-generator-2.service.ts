...
import { GeneratedEncounter } from "src/app/components/encounters/models/encounter";
import type { EncounterRequest } from "src/app/components/encounters/models/encounter-request";
...
export class EncounterGenerator2Service {
  ...
  private readonly _previousEncounters$ = new BehaviorSubject(
    this._previousEncounters
  );
  private readonly maxPreviousEncountersLength = 20;

  public readonly previousEncounters$ =
    this._previousEncounters$.asObservable();
  ...
  constructor(
    ...
    const previouslyGeneratedEncounters = storage.getData<GeneratedEncounter[]>(
      StorageKeys.PreviouslyGeneratedEncounters
    );
  ...
  public async generateEncounter(
    ...
    filter: MonsterFilters
  ): Promise<GeneratedEncounter> {
    const crToExp = crToExpMap.get(request.level)!;
    const numberOfPlayersModifier = request.numberOfPlayers * 0.25;
    const targetExp =
      crToExp * numberOfPlayersModifier * request.difficultyAmount;
    ...
    if (filteredMonsters.length === 0) {
      throw new Error(
        "There are no available monsters that match user filters"
      );
    }
    const encounters = [];
    for (let index = 0; index < this.numberOfEncountersToGenerate; index++) {
      encounters.push(
        this.createEncounter(filteredMonsters, targetExp, request)
      );
    }
    const newEncounter = this.findEncounterClosestToTargetExp(
      encounters,
      targetExp
    );
  ...
  private createEncounter(
    ...
    do {
      ...
      monstersForEncounter.length < request.maxNumberOfEnemies &&
  ...
  private findEncounterClosestToTargetExp(
    possibleEncounters: GeneratedEncounter[],
    expTarget: number
  ) {
    return possibleEncounters.reduce(
      (currentClosestEncounter, nextEncounterToTest) => {
        ...
        if (aDiff == bDiff) {
          ...
          return currentExp > beingTestedExp
            ? currentClosestEncounter
            : nextEncounterToTest;
        } else {
  ...
  private updateEncounters(newEncounter: GeneratedEncounter) {
    ...
    this._previousEncounters = this._previousEncounters.slice(
      0,
      Math.min(newLength, this.maxPreviousEncountersLength)
    );
    this._previousEncounters$.next(this._previousEncounters);
